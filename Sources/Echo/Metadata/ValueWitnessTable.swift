...
public struct ValueWitnessTable: LayoutWrapper {
    ...
    public func initializeBufferWithCopyOfBuffer(
        ...
        #if _ptrauth(_arm64e)
            echo_vwt_initializeBufferWithCopyOfBuffer(
                layout.signed, dest, source,
                trailing)
        #else
    ...
    public func getEnumTagSinglePayload(
        ...
        #if _ptrauth(_arm64e)
            return echo_vwt_getEnumTagSinglePayload(
    ...
    public func storeEnumTagSinglePayload(
        ...
        #if _ptrauth(_arm64e)
            echo_vwt_storeEnumTagSinglePayload(
                layout.signed, instance, tag,
                numEmptyCases, trailing)
        #else
...
struct _ValueWitnessTable {
    let _initializeBufferWithCopyOfBuffer:
        @convention(c) (
            // Destination buffer
    ...
    let _storeEnumTagSinglePayload:
        @convention(c) (
            // Instance of single payload enum
            UnsafeMutableRawPointer,
            // Which enum case
